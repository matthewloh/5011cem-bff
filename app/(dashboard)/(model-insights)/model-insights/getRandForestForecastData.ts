import { RFRData } from "@/api/model_outputs/rfr_forecasts";
...
export async function getRFRForecastData(
  ...
  // Recorded after is always "2024-04-21"
  let numOfDays = differenceInDays(
    recordedBefore || new Date(),
    new Date("2024-04-20"),
  );
  if (numOfDays == 0 || numOfDays < 0) {
    numOfDays = 90;
  }

  const finalDate = addDays(new Date("2024-04-20"), numOfDays + 1)
    .toISOString()
    .split("T")[0];
  const finalString = `2024-04-21_${finalDate}`;
  const data = RFRData[`2024-04-21_${finalDate}` as keyof typeof RFRData];
